...
def main():
    ...
    # Display the result
    st.write("Thanks for your information. Below are some suggestions:")
    # Clear Button
    if st.button("Clear"):
...
def show_kid_hobbies_sub_question(kid_hobbies_value):
    ...
    if kid_hobbies_value == "Books":
        ...
        kid_sport_type = st.selectbox("What kind of sport do they play?",
                                      ["", "biking", "running", "trekking", "swimming",
...
def search_book(book_genre, budget_range):
    ...
    if len(filtered_books) < 1:
        st.write("Not enough items to display. Please adjust your search criteria.")
    else:
...
def search_game(game_type, age, budget_range):
    ...
    # Select a random sample of 10 books from the filtered DataFrame
    random_game = filtered_game.sample(n=10, random_state=42)

    st.write("Search Results:")
    st.write(random_game)
...
def search_sport(sport_type, gender, budget_range):
    ...
    # Set the value of collection based on gender
    collection_value = (
        'kid' if gender == 'A Kid' else
        'woman' or 'adult' if gender == 'A Woman' else
        'man' or 'adult' if gender == 'A Man' else None
    )
